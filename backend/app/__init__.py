...
def create_app():
    ...
    # ⬇️ INICIALIZAR O RAG AQUI DENTRO (quando a app é criada)
    from .rag import criar_chain_rag  # ⬅️ Importar aqui dentro
    qa_chain = criar_chain_rag()
    
    if not qa_chain:
        raise RuntimeError("❌ Falha ao inicializar RAG. Execute python vetorizador.py primeiro!")
